#Temp: 50 - 1000 deg C, increments of 10
#baketime: 1e1 - 1e10 seconds, increments of an order of magnitude
#t_res: 25 - 500 days, increments by 25
...
for t_res_day in range(25,501,25):
    ...
    data = np.genfromtxt(results_folder + f'/pebble{t_res_day}.txt', skip_header=1, delimiter=",")
    full = data[:,0]
    ...
    for temp in range(50,1001,100):
        for time_exp in range(100,600,25):
            time = 10**(time_exp/100)
            step = time/5
            ...
            my_model.exports = [
                F.XDMFExport(
                    field="0",
                    filename=results_folder + f'/oven_{t_res_day}_{temp}_1e{time_exp}.xdmf',
                    checkpoint=False  # needed in 1D
                    ),
                F.TXTExport(field="0", times=t.tolist(), filename=results_folder+f'/oven_{t_res_day}_{temp}_1e{time_exp}.txt')
            ]
            ...
            data = np.genfromtxt(
                results_folder + f'/oven_{t_res_day}_{temp}_1e{time_exp}.txt', skip_header=1, delimiter=","
            )
            xdata = data[:,0]
            initialdata = data[:, 1]
            finaldata = data[:, len(data[0])-1]
            N = np.trapz(y = initialdata, x = xdata) - np.trapz(y = finaldata, x = xdata)
            datapoint = np.array([t_res_day, temp, time_exp, N])
            f = open("\oven_output.txt", "a")
            f.write(f'{datapoint} \n')
